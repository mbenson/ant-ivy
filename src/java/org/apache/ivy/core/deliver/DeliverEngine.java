...
public class DeliverEngine {
    ...
    public void deliver(ModuleRevisionId mrid, String revision, String destIvyPattern,
        ...
        Map<ModuleRevisionId, String> resolvedRevisions = new HashMap<ModuleRevisionId, String>();
        Map<ModuleRevisionId, String> resolvedBranches = new HashMap<ModuleRevisionId, String>();
        Map<ModuleRevisionId, String> dependenciesStatus = new HashMap<ModuleRevisionId, String>();
            ...
            if (options.isReplaceForcedRevisions()) {
                if (parts.length <= 2) {
                    ...
                    throw new IllegalStateException(
                            "ivy properties file generated by an older version of Ivy which doesn't support replacing forced revisions!");
                }
        ...
        for (DependencyDescriptor dependency : dependencies) {
            ...
            if (bra == null) {
                mrid2 = ModuleRevisionId.newInstance(dependency.getDependencyRevisionId(), rev);
            } else {
                mrid2 = ModuleRevisionId.newInstance(dependency.getDependencyRevisionId(), bra,
                    rev);
            }
        ...
        Set<String> confsToRemove = new HashSet<String>(Arrays.asList(md.getConfigurationsNames()));
        confsToRemove.removeAll(Arrays.asList(confs));

        try {
            UpdateOptions opts = new UpdateOptions().setSettings(settings)
                    .setResolvedRevisions(resolvedDependencies).setStatus(options.getStatus())
                    .setRevision(revision).setBranch(options.getPubBranch())
                    ...
                    .setConfsToExclude(confsToRemove.toArray(new String[confsToRemove.size()]));
